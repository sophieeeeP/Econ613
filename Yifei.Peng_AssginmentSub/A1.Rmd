...
```
## libraries loading, working path setting and data input
```{r eval = FALSE}
...
```
Notes: 
1 three way to turn a matrix into vector
...
as.vector()
```
2 How to use col in dataframe? (Dataframe$col)
3 data2 <- gather(data, category, value, X, Y, Z)
...
```
possible issure:statisticsglobe.com/r-error-cant-rename-columns-that-dont-exist
...
report avg, sd of cutoff, quality, dist for each ranked choice
```{r, echo=TRUE}
...
report1
```
redo the table, differentiating by student test score quatiles
```{r, echo=TRUE}
...
r
```
It is much smaller than 1.2, the difference between this two around 1. When 1.2 is the coefficient of X1 with Y, 0.2 means the 
...
summary(linearprob)
```
interpre and compare estimated coeff, how sig are they? 
* In the probit model, **X1** and **X2** are statistically significant, which **X3** is significant at 99% level. 
  + For one unit increase in **X1**, the y increases by 118% 
  + For one unit increase in **X2**, the y decreases by 90.854%  
  + For one unit increase in **X3**, the y increases by 9.062% 

* In the logit model, **X1** and **X2** are statistically significant, which **X3** is significant at 99% level.
  + For one unit increase in **X1**, the y increases by 2.12066 
  + For one unit increase in **X2**, the y decreases by 1.63618 
  + For one unit increase in **X3**, the y increases by 0.17604

* In the linear probability model, **X1** and **X2** are statistically significant, which **X3** is significant at 99% level.
  + For one unit increase in **X1**, the y iecreases by 0.1362626  
  + For one unit increase in **X2**, the y increases by 0.1043247 
  + For one unit increase in **X3**,  the y increases by 0.0156355

* From above results, the estimated coefficient reached by different models varies a little bit from each other. However, they are showing similar relationfor overall data. 

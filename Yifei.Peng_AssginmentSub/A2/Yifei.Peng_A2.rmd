...
hotkey to add chuck: crtl +alt +I 


1. Average and dispersion in product characteristics(price)
```{r, include=FALSE}
...
```
 ...
 2. Market share (choice frequency) and market share by product characteristics (choice frequency by price bins: below average, over average).
 * Market share: Table of Choice Variable
```{r }
...
```
 ...
 3.  Illustrate the mapping between observed attributes and choices (Which customers are choosing which products?) :  see some individuals with specific characteristics choosing some products with specific attributes? make some tables..
```{r}
map = d1[,1:12] %>% group_by(hhid,choice) %>% summarize(count = n())
map
```
* Observing 5 randomly selected households purchasing record, which the household has more than 5 records.
```{r}
...
```
By the table presented above, first of all, every household have choice loyalty, i.e. they would always purchase one product more often than any other products. Besides, even the price changes,it would affect the purchasement too much. 

## Exercise 2 First Model
Hints:Hi professor, also, for exercise 2 and 3, do we have to build the model from scratch?If by scratch, you mean writing the likelihood.. the answer is YES

* We are interested in the effect of price on demand. Propose a model specification.

Since the choice is a discrete variable. We could build a conditional logit model. 
```{r}
...
```
don't know use which model. Is it 
```{r}
...
We want to use a multinomial logit model for measure the effect of family income on demand.
```{r}
d4 <- margarine$demo
d4 <- d4[,1:2]
...
* We are still interested in the effect of price and family income. Write and optimize the likelihood of the mixed logit. Denote by β^f the estimated coefficients.
```{r}
...
* Consider an alternative specification, where we remove data from one choice. Estimate this model as well, and denote by β^r the estimated parameters.
